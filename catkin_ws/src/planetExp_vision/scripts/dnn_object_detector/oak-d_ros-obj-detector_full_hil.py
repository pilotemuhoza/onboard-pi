...
class vision_processor:
    def __init__(self, args):
        ...
        # self.camera_model = GenericCamera(self.focalLength,
        #                                   self.sensorWidth,
        #                                   self.sensorHeight,
        #                                   self.widthRes,
        #                                   self.heightRes,
        ...
        with dai.Device(pipeline) as device:
            ...
            while not rospy.is_shutdown():
                ...
                rospy.loginfo_once("Performing post-processing tasks ...")
                # if (self.detected_target):
                #     tgPos = self.targetPoseEstimation()
                #     self.targetPose.pose.position.x = tgPos[0]
                #     self.targetPose.pose.position.y = tgPos[1]
                #     self.targetPose.pose.position.z = tgPos[2]
        ...
        if self.cv_image is not None:
            ...
            if out_nn is not None:
                ...
                if 'road' in self.nn_path or 'Unet' in self.nn_path or 'DeepLab' in self.nn_path or 'res18_skip' in self.nn_path:
                    ...
                    layer1 = out_nn.getLayerInt32(layers[0].name)
                ...
                self.publish_raw_frame(self.net_publisher, detection_mask)

                # Update resolution camera property based on processed image
                if self.is_res_updated is False:
                    # self.camera_model.set_res_x(detection_mask.shape[1])
                    # self.camera_model.set_res_y(detection_mask.shape[0])
                ...
                if len(contours) > 0:
                    ...
                    self.publish_detection_rate(
                        self.net_detectionRate, confidence)
                    self.publish_detection(
                        self.net_detection, detectedTarget)
